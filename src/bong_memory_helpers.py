...
_BOILERPLATE = re.compile(r"\bbong['']?s?\b", re.IGNORECASE)
...
def _apply_recency_boost(score: float, saved_at: float, halflife_days: float = RECENCY_HALFLIFE_DAYS) -> float:
    if not saved_at:
        return score
...
def retrieve_memories(query: str, username: str = "", user_id: int | None = None, k: int = 10) -> str:
    try:
        ...
        cleaned_name = _clean_for_embedding(username) if username else ""

        searches = []
        is_user_search = []
        if user_id:
            searches.append(_vector_db.similarity_search_with_relevance_scores(
                cleaned_query, k=k, filter={"user_id": user_id}
            ))
            is_user_search.append(True)
        searches.append(_vector_db.similarity_search_with_relevance_scores(cleaned_query, k=k))
        is_user_search.append(False)
        if cleaned_name:
            searches.append(_vector_db.similarity_search_with_relevance_scores(cleaned_name, k=k))
            is_user_search.append(False)
        ...
        for search_docs, from_user_search in zip(searches, is_user_search):
            for doc, score in search_docs:
                ...
                adjusted_score = score * (1.0 + USER_MEMORY_SCORE_BOOST) if from_user_search else score
                saved_at = doc.metadata.get("saved_at")
                if saved_at:
                    adjusted_score = _apply_recency_boost(adjusted_score, saved_at)
        ...
        formatted = []
        for doc, _, _ in sorted(all_results, key=lambda x: x[2], reverse=True):
            meta_parts = []
            saved_at = doc.metadata.get("saved_at")
            if saved_at:
                try:
...
def _find_contradiction(fact: str, user_id: int | None) -> str | None:
    try:
        candidates = []
        if user_id:
            similar = _vector_db.similarity_search_with_relevance_scores(
        ...
        if not candidates:
            similar_general = _vector_db.similarity_search_with_relevance_scores(fact, k=5)
